...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        print("Using local version of GPT2LMHeadModel")
...
class DoublePositionalGPT2Embeddings(nn.Module):
    def __init__(self, config):
        super(DoublePositionalGPT2Embeddings, self).__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.next_position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.embd_pdrop)

    def forward(self, input_ids, position_ids=None, next_position_ids=None):
        input_shape = input_ids.size()
        ...
        if next_position_ids is None:
            next_position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
            next_position_ids = next_position_ids.unsqueeze(0).expand(input_shape)

        inputs_embeds = self.word_embeddings(input_ids)
        position_embeddings = self.position_embeddings(position_ids)
        next_position_embeddings = self.next_position_embeddings(next_position_ids)

        embeddings = inputs_embeds + position_embeddings + next_position_embeddings
...
class CustomGPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.wte = DoublePositionalGPT2Embeddings(config)
        self.drop = nn.Dropout(config.embd_pdrop)
        self.h = nn.ModuleList([GPT2Block(config, layer_idx=i) for i in range(config.num_hidden_layers)])
        self.ln_f = nn.LayerNorm(self.embed_dim, eps=config.layer_norm_epsilon)
    ...
    def get_input_embeddings(self):
        return self.wte.word_embeddings

    def set_input_embeddings(self, new_embeddings):
        self.wte.word_embeddings = new_embeddings
    ...
    def forward(
            ...
            token_type_ids: Optional[torch.LongTensor] = None,
            position_ids: Optional[torch.LongTensor] = None,
            next_position_ids: Optional[torch.LongTensor] = None,
            head_mask: Optional[torch.FloatTensor] = None,
            inputs_embeds: Optional[torch.FloatTensor] = None,
            encoder_hidden_states: Optional[torch.Tensor] = None,
            encoder_attention_mask: Optional[torch.FloatTensor] = None,
        ...
        if next_position_ids is None:
            next_position_ids = (position_ids + 1) % input_shape[-1]

        if inputs_embeds is None:
            inputs_embeds = self.wte(input_ids, position_ids=position_ids, next_position_ids=next_position_ids)
        else:
            inputs_embeds = self.wte(inputs_embeds, position_ids=position_ids, next_position_ids=next_position_ids)
        ...
        if token_type_ids is not None:
            token_type_embeds = self.wte.word_embeddings(token_type_ids)
            hidden_states = hidden_states + token_type_embeds
